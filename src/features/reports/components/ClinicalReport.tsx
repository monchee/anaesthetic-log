      ...
      <div className="border-l-4 border-primary bg-slate-50 dark:bg-card/30 p-4 md:p-6 print:bg-white print:border-l-0 print:p-2">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <h1 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-foreground">Anaesthetic Testing Report</h1>
            <p className="text-sm text-slate-600 dark:text-muted-foreground mt-1">Clinical Immunology & Allergy · Royal Prince Alfred Hospital</p>
          </div>
          <div className="text-right">
            <p className="text-xs uppercase tracking-wider text-slate-500 dark:text-muted-foreground">Generated</p>
            <p className="text-sm font-semibold text-slate-900 dark:text-foreground">{new Date().toLocaleDateString('en-AU')}</p>
          </div>
        </div>
      </div>
      ...
      <CardContent className="p-4 md:p-8 lg:p-12 space-y-8 md:space-y-10 print:p-3 print:space-y-3">
         ...
         <div className="bg-slate-50 dark:bg-card/30 border border-slate-200 dark:border-border rounded-lg p-4 print:bg-white print:border-slate-300">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 print:grid-cols-2 print:gap-2">
            <div>
               <label className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-muted-foreground font-semibold block mb-1 print:mb-0.5">Patient Name</label>
               <p className="text-xl md:text-2xl font-semibold tracking-tight text-primary print:text-base">{data.firstName} {data.lastName}</p>
            </div>
            <div>
               <label className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-muted-foreground font-semibold block mb-1 print:mb-0.5">MRN</label>
               <p className="text-lg font-mono font-medium text-slate-900 dark:text-foreground print:text-sm">{data.mrn}</p>
            </div>
            <div className="md:col-span-2 print:col-span-2">
               <label className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-muted-foreground font-semibold block mb-1 print:mb-0.5">Visit Date</label>
               <p className="text-lg font-medium text-slate-900 dark:text-foreground print:text-sm">{formatDate(data.visitDate)}</p>
            </div>
           </div>
         </div>
         ...
         <div>
            <h3 className="text-sm md:text-base font-bold text-slate-900 dark:text-foreground uppercase tracking-wider flex items-center gap-2 border-b-2 border-primary pb-2 mb-4 print:text-xs print:mb-2">
              <Activity className="w-5 h-5 print:w-4 print:h-4" /> Skin & Intradermal Testing
            </h3>
            
            {/* Controls */}
            <div className="bg-slate-50 dark:bg-card/30 border border-slate-200 dark:border-border rounded-lg p-3 text-sm mb-4 print:bg-white print:border-slate-300 print:p-2 print:mb-2 print:text-xs">
            ...
            {(data.testPanel || []).length > 0 ? (
              <>
                ...
                <div className="md:hidden print:hidden space-y-3">
                   {data.testPanel.map((row, i) => (
                      <div key={i} className="bg-slate-50 dark:bg-card/50 border border-slate-200 dark:border-border rounded-none p-4">
                          <div className="font-bold text-slate-900 dark:text-foreground mb-3 border-b border-slate-200 dark:border-border pb-2">
                             {row.drugName === 'Other' ? (row.customName || 'Other') : row.drugName}
                          </div>
                          <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                              <div>
                                 <span className="text-[10px] text-slate-500 dark:text-muted-foreground uppercase font-bold block">SPT</span>
                          ...
                          {row.notes && (
                              <div className="mt-2 pt-2 border-t border-slate-200 dark:border-border text-xs text-muted-foreground">{row.notes}</div>
                          )}
                      </div>
                   ))}
                </div>
         ...
         <div>
            ...
            {data.proceedToChallenge ? (
               <div className={`border-l-4 p-4 rounded-lg bg-white dark:bg-card ${data.outcome === 'SUCCESS' ? 'border-green-500' : 'border-red-500'} print:bg-white print:border-l-2 print:p-2`}>
         ...
         <div className="pb-4 print:pb-2">
            <h3 className="tracking-tight text-lg mb-2 flex items-center gap-2 text-slate-900 dark:text-foreground print:text-sm print:mb-1">
              <FileText className="w-5 h-5 print:w-4 print:h-4" /> Assessment & Plan
            </h3>
            <div className="bg-slate-50 dark:bg-card/30 border border-slate-200 dark:border-border rounded-lg p-4 whitespace-pre-wrap text-sm md:text-base print:bg-white print:border-slate-300 print:p-2 print:text-xs">
