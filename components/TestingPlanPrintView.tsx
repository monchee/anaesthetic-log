        ...
        <CardContent className="p-8 md:p-12 print:p-2">
            ...
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-md grid grid-cols-2 gap-4 mt-8 print:bg-slate-50 print:p-2 print:mt-2 print:gap-2">
                ...
                <div>
                    ...
                    <p className="text-lg font-mono font-medium text-slate-700 print:text-xs">{patient.mrn}</p>
                </div>
                <div>
                        <p className="text-[10px] uppercase font-semibold text-slate-500 tracking-wider print:text-[9px]">DOB</p>
                        <p className="text-slate-700 font-medium print:text-xs">{formatDate(patient.dob)}</p>
                </div>
                <div>
            ...
            {notes && (
                <div className="mt-8 print:mt-2">
                    <h4 className="font-semibold text-slate-800 border-b border-slate-100 mb-2 uppercase text-[11px] tracking-wider print:text-[10px] print:mb-1 print:pb-0.5">Clinical Notes</h4>
                    <p className="text-slate-700 whitespace-pre-wrap text-sm print:text-xs">{notes}</p>
                </div>
            )}

            {/* Selected Drugs List */}
            <div className="mt-8 print:mt-2">
                <h4 className="font-semibold text-slate-800 border-b-2 border-slate-800 mb-4 uppercase text-[11px] tracking-wider flex items-center gap-2 print:text-[10px] print:mb-2 print:pb-1 print:border-b">
                        ...
                        {Object.entries(drugCategories).map(([category, drugs]) => {
                            ...
                            return (
                                <div key={category} className="break-inside-avoid mb-4 print:mb-1">
                                    <h5 className="font-semibold text-slate-600 text-xs uppercase mb-2 print:text-[10px] print:mb-0.5">{category}</h5>
